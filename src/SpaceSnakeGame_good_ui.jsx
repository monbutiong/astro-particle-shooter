...
const useSoundManager = () => {
  ...
  const initSounds = useCallback(async () => {
    ...
    const soundPaths = {
     ...
     // Power-up sounds
     powerUp: '/audio/fireworks.wav',
     powerUpSpecial: '/audio/fireworks.wav',
     jackpot: '/audio/fireworks.wav',
     
     // Game state sounds
     gameOver: '/audio/game-over.wav',
     newRecord: '/audio/fireworks.wav',
     winFireworks: '/audio/fireworks.wav',
...
const SpaceSnakeGame = () => {
  ...
  // CRITICAL FIX: Singleton pattern - only call useSoundManager ONCE per component lifecycle
  const soundManagerRef = useRef(null);
  if (!soundManagerRef.current) {
    soundManagerRef.current = useSoundManager();
  }
  const musicStartedRef = useRef(false);
   ...
   // Load ship image when character is selected
   useEffect(() => {
     if (selectedCharacter && playerConfigs[selectedCharacter]) {
       const img = new Image();
       img.src = playerConfigs[selectedCharacter].shipImage;
       img.onload = () => {
         shipImageRef.current = img;
       };
       img.onerror = () => {
         console.error('Failed to load ship image:', playerConfigs[selectedCharacter].shipImage);
  ...
  useEffect(() => {
    ...
    if (savedPlayer) {
      try {
        ...
        // Only skip login if we have both name AND character
        if (playerData.name && playerData.character) {
          setShowLoginScreen(false);
          setShowCharacterSelect(false);
        } else if (playerData.name) {
          // Have name but no character, skip login but show character select
          setShowLoginScreen(false);
 ...
 const [bossTimerDisplay, setBossTimerDisplay] = useState(60);
...
// Power-up types configuration
  ...
  const update = (deltaTime, canvas) => {
    ...
    if (player.autoFire || keysRef.current[' ']) {
      if (now - player.lastShot > fireRate) {
        ...
        if (player.superMode) {
          ...
          soundManagerRef.current.playSound('shoot');
          bulletsRef.current.push(
    ...
    // Check level progression
        setTimeout(() => {
          const newLevel = (levelRef.current % 10) + 1;
          levelRef.current = newLevel;
          soundManagerRef.current.playMusic(newLevel);
          startLevel(newLevel);
        }, 2000);
      }
    ...
    switch (type) {
      ...
      case 'BOMB':
        ...
        enemiesRef.current.forEach(enemy => {
          ...
          bombScore += enemy.type.score;
        });
        enemiesRef.current = [];
        setTimeout(() => setScore(prev => prev + bombScore), 0);
        break;
      case 'SUPER':
        ...
        break;
      case 'EXTRA_LIFE':
        setTimeout(() => setLives(prev => Math.min(prev + 1, 9)), 0);
        break;
    }
  };
  ...
  const addPowerUpIndicator = (config) => {
    ...
    // Remove expired power-up after duration
    setTimeout(() => {
  ...
  const draw = (ctx, canvas) => {
    ...
    enemyBulletsRef.current.forEach(bullet => {
     ...
     if (player) {
       ...
       // Draw ship image or fallback to default ship
       if (shipImageRef.current) {
         const shipSize = player.width * 1.5;
         ctx.drawImage(
  ...
  return (
    <div className="relative w-full h-screen bg-slate-950 overflow-hidden">
          ...
          {bossWarningRef.current && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <div className="text-xl font-bold text-red-500 animate-pulse title-text" >
                  ⚠️ BOSS INCOMING! ⚠️
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-sm font-bold rounded-lg hover:from-cyan-400 hover:to-blue-500 transition-all transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {showCharacterSelect && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 z-50">
            ...
            <div className="flex justify-center gap-4 pt-4">
              <div className="flex gap-3 w-full">
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-slate-700 text-white font-bold rounded-lg hover:bg-slate-600 transition-all border border-slate-600"
                >
                  ← Back
                </button>
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold rounded-lg hover:from-cyan-400 hover:to-blue-500 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div className="text-center space-y-6 p-8 bg-gradient-to-b from-slate-900 to-slate-950 rounded-2xl border border-cyan-500/30 shadow-2xl max-w-lg">
            ...
            <div className="flex items-center justify-center gap-4 bg-slate-800/50 rounded-xl p-4 border border-cyan-500/20">
              <div className="relative w-16 h-16 flex-shrink-0 overflow-hidden rounded-lg bg-slate-900 border-2 border-slate-600 shadow-lg">
                <img
                  src={playerConfigs[selectedCharacter]?.avatarImage}
     ...
     {gameState === 'playing' && (
       <div className="absolute top-0 left-0 right-0 p-4 z-10 pointer-events-none">
         <div className="max-w-7xl mx-auto space-y-3">
           {/* Top Navigation Bar */}
           <div className="bg-gradient-to-r from-slate-900/95 to-slate-800/95 backdrop-blur-sm rounded-2xl p-3 border border-cyan-500/30 shadow-2xl">
             <div className="flex justify-between items-center">
               {/* Left: Game Title */}
               <div className="flex items-center gap-3">
                 <h1 className="title-text text-2xl font-bold bg-gradient-to-r from-cyan-400 to-blue-600 bg-clip-text text-transparent">
                   SPACE SNAKE
                 </h1>
               </div>
               ...
               <div className="flex gap-3">
                 <div className="text-center">
                   <p className="text-yellow-400 font-bold text-3xl title-text leading-none">{score}</p>
                   <p className="text-gray-400 text-xs">SCORE</p>
                 </div>
                 <div className="text-center">
                   <p className="text-purple-400 font-bold text-2xl title-text leading-none">{level}</p>
               ...
               <div className="flex gap-3 items-center">
                 ...
                 {activePowerUps.length > 0 && (
                   <div className="flex items-center gap-2">
                     ...
                     <span className="text-white font-bold text-lg">{activePowerUps.length}</span>
                   </div>
                 )}
               </div>
             </div>
           ...
           {/* Second Row: Player Info and Boss Info */}
           <div className="flex justify-between items-start gap-3">
             {/* Left: Player Info with Avatar */}
             <div className="bg-slate-900/90 backdrop-blur-sm rounded-2xl p-3 border border-cyan-500/30 shadow-2xl flex-1">
               <div className="flex items-center gap-3">
                 {/* Player Avatar */}
                 <div className="relative w-14 h-14 flex-shrink-0 overflow-hidden rounded-xl bg-slate-800 border-2 border-slate-600 shadow-lg">
                   <img
                     src={playerConfigs[selectedCharacter]?.avatarImage}
                     alt={playerConfigs[selectedCharacter]?.name}
                     className="w-full h-full object-cover"
                 ...
                 <div className="text-left flex-1">
                   <p className="text-white font-bold text-base leading-tight">{playerName}</p>
                   <p className="text-xs font-bold" >
                     {playerConfigs[selectedCharacter]?.emoji} {playerConfigs[selectedCharacter]?.name ?? 'Pilot'}
                   </p>
                 </div>
               </div>
             </div>

             {/* Center: Boss/Enemy Info */}
             <div className="bg-slate-900/90 backdrop-blur-sm rounded-2xl px-4 py-3 border border-orange-500/30 shadow-2xl flex-1 text-center">
               {!bossRef.current ? (
                 <div>
                   <p className="text-gray-400 text-xs">BOSS INCOMING</p>
                   <p className="text-orange-400 font-bold text-2xl title-text leading-none">
                     ⏱️ {bossTimerDisplayRef.current}s
                   </p>
                 </div>
               ) : (
                 <div>
                   <p className="text-red-400 text-xs font-bold">⚠️ BOSS BATTLE</p>
                   <p className="text-white font-bold text-sm leading-tight">
                     {bossRef.current.stage.name}
                   </p>
                   <div className="mt-1 w-full bg-slate-800 rounded-full h-2 overflow-hidden">
                     <div 
