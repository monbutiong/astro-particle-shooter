...
const useSoundManager = () => {
  ...
  const initSounds = useCallback(async () => {
    ...
    const soundPaths = {
      // Shooting sounds
      shoot: '/assets/audio/fire.wav',
      shootNormal: '/assets/audio/fire-normal.wav',
      
      // Enemy sounds
      enemyHit: '/assets/audio/enemy-hit.wav',
      enemyDestroy: '/assets/audio/fireworks.wav',
      pinHit: '/assets/audio/pin_hit.wav',
      slotHit: '/assets/audio/slot_hit.wav',
      
      // Boss sounds
      bossDead: '/assets/audio/boss dead.wav',
      
      // Power-up sounds
      powerUp: '/assets/audio/power-ups.wav',
      powerUpSpecial: '/assets/audio/power-ups-1.wav',
      jackpot: '/assets/audio/jackpot.wav',
      
      // Game state sounds
      uiClick: '/assets/audio/ui_click.wav',
      gameOver: '/assets/audio/game_over.wav',
      newRecord: '/assets/audio/new_record.wav',
      winFireworks: '/assets/audio/win_fireworks.wav',
      
      // Background music
      bgmLevel1: '/assets/audio/background-sound-1-to-10.mp3',
      bgmLevel2: '/assets/audio/background-sound-11-to-20.mp3',
      bgmLevel3: '/assets/audio/background-sound-21-to-30.mp3',
      bgmLevel4: '/assets/audio/background-sound-31-to-all.mp3',
    };
...
const SpaceSnakeGame = ({ playerName, character, onBackToMenu, onChangeCharacter }) => {
  const canvasRef = useRef(null);
  const gameLoopRef = useRef(null);
  const keysRef = useRef({});
  const spaceshipImageRef = useRef(null);
  
  const [gameState, setGameState] = useState('playing'); // Skip start screen, go directly to playing
  const [isPaused, setIsPaused] = useState(false); // Track if game is paused (idle/minimized)
  ...
  // Avatar state for dynamic emotions
  const [avatarState, setAvatarState] = useState('ready'); // ready, scared, crying
  const lastHitTimeRef = useRef(0);
 
  // Reset avatar state after 2 seconds of no damage
  useEffect(() => {
    if (avatarState !== 'ready' && lastHitTimeRef.current > 0) {
      const resetTimer = setTimeout(() => {
        if (Date.now() - lastHitTimeRef.current >= 2000 && lives > 1) {
          setAvatarState('ready');
        }
      }, 2100); // Slightly longer than 2 seconds
...
// Power-up types configuration
  ...
  const initGame = useCallback(() => {
    ...
    if (character?.bonuses) {
      // Apply speed bonus
      if (character.bonuses.speed) {
        playerRef.current.speed = 8 * character.bonuses.speed;
      }
      
      // Apply fire rate bonus
      if (character.bonuses.fireRate) {
        playerRef.current.fireRate = 200 * character.bonuses.fireRate;
      }
      
      // Apply spread shot for pink character
      if (character.bonuses.spreadShot) {
        playerRef.current.spreadShot = true;
      }
      
      // Apply extra starting lives
      let startingLives = 3;
      if (character.bonuses.lives) {
        startingLives += character.bonuses.lives;
      }
      
      // Set lives after character bonuses
      setTimeout(() => setLives(startingLives), 0);
    }
    ...
    bossTimerDisplayRef.current = 60;
      ...
      // Load spaceship image for player and boss
      if (character?.id) {
        const spaceshipImagePath = `/assets/player/${character.id}-ship.fw.png`;
        spaceshipImageRef.current = new Image();
        spaceshipImageRef.current.src = spaceshipImagePath;
        spaceshipImageRef.current.onload = () => {
          console.log('Spaceship image loaded:', spaceshipImagePath);
        };
        spaceshipImageRef.current.onerror = () => {
          console.warn('Failed to load spaceship image:', spaceshipImagePath);
          spaceshipImageRef.current = null;
        };
      }
    ...
    // Initialize audio immediately (non-blocking)
    if (!audioInitializedRef.current) {
      soundManager.initAudio();
      // Don't await - let audio load in background
      soundManager.initSounds().catch(err => console.warn('Audio load warning:', err));
      audioInitializedRef.current = true;
    }
  }, [character, soundManager]);
  ...
  // Handle visibility change (minimized/tab hidden)
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && gameState === 'playing') {
        // Pause game when tab is hidden/minimized
        if (gameLoopRef.current) {
          cancelAnimationFrame(gameLoopRef.current);
        }
        // Pause music by accessing sounds through soundManager
        const bgmKeys = ['bgmLevel1', 'bgmLevel2', 'bgmLevel3', 'bgmLevel4'];
        for (const key of bgmKeys) {
          const music = soundManager?.soundsRef?.current?.[key];
          if (music) {
            music.pause();
            break;
          }
        ...
        setIsPaused(true);
      } else if (!document.hidden && gameState === 'playing' && isPaused) {
  ...
  useEffect(() => {
    ...
    idleTimer = setInterval(() => {
      ...
      if (idleTime >= IDLE_THRESHOLD && !isPaused && gameState === 'playing') {
    ...
    enemyBulletsRef.current = enemyBulletsRef.current.filter(bullet => {
      ...
      if (!player.hasShield && !player.superMode) {
        ...
        if (dist < player.width / 2 + bullet.size) {
          ...
          // Update avatar state on hit
          setAvatarState('scared');
          lastHitTimeRef.current = Date.now();
          setTimeout(() => {
            setLives(prev => {
              const newLives = prev - 1;
              // Set to crying if on last life
              if (newLives === 1) {
  ...
  const draw = (ctx, canvas) => {
    ...
    if (bossRef.current) {
      ...
      // Draw boss shape based on type - each stage has unique appearance
      ctx.globalAlpha = 1;
      
      // Draw unique boss shape based on stage
      const bossSize = boss.stage.size;
      
      ctx.fillStyle = boss.stage.secondaryColor;
      ctx.strokeStyle = boss.stage.color;
      ctx.lineWidth = 4;
      ctx.shadowBlur = 30;
      ctx.shadowColor = boss.stage.color;
      
      // Draw different shapes based on boss stage
      switch (boss.stage.shape) {
        case 'circle':
          // Stage 1: Circle with rotating orbiting particles
          ctx.beginPath();
          ...
          // Add orbiting rings
          ctx.beginPath();
          ctx.arc(0, 0, bossSize * 0.7, 0, Math.PI * 2);
          ctx.strokeStyle = boss.stage.secondaryColor;
          ctx.lineWidth = 2;
          ctx.stroke();
          break;
          
        case 'square':
          // Stage 2: Rotating square with inner core
          ctx.save();
          ctx.rotate(boss.time * 0.001);
          ctx.fillRect(-bossSize * 0.5, -bossSize * 0.5, bossSize, bossSize);
          ctx.strokeRect(-bossSize * 0.5, -bossSize * 0.5, bossSize, bossSize);
          ctx.restore();
          // Inner core
          ctx.beginPath();
          ctx.arc(0, 0, bossSize * 0.2, 0, Math.PI * 2);
          ctx.fillStyle = boss.stage.color;
          ctx.fill();
          break;
          
        case 'triangle':
          // Stage 3: Triangle with pulsing effect
          const pulseSize = 1 + Math.sin(boss.time * 0.003) * 0.1;
          ctx.beginPath();
          ctx.moveTo(0, -bossSize * 0.6 * pulseSize);
          ctx.lineTo(-bossSize * 0.5, bossSize * 0.4);
          ctx.lineTo(bossSize * 0.5, bossSize * 0.4);
          ctx.closePath();
          ctx.fill();
          ctx.stroke();
        ...
        case 'diamond':
          // Stage 4: Diamond with multiple layers
          for (let i = 0; i < 3; i++) {
            ctx.beginPath();
            const layerSize = bossSize * (0.3 + i * 0.2);
        ...
        case 'hexagon':
          // Stage 5: Hexagon with pulsating vertices
          ctx.beginPath();
          for (let i = 0; i < 6; i++) {
            const angle = (Math.PI * 2 / 6) * i - Math.PI / 2;
            const pulseOffset = Math.sin(boss.time * 0.005 + i) * 10;
            const radius = bossSize * 0.5 + pulseOffset;
        ...
        case 'star':
          // Stage 6: Star with rotating rays
          ctx.save();
          ctx.rotate(-boss.time * 0.0005);
          ctx.beginPath();
          for (let i = 0; i < 12; i++) {
            const angle = (Math.PI * 2 / 12) * i - Math.PI / 2;
            const radius = i % 2 === 0 ? bossSize * 0.6 : bossSize * 0.3;
            const x = Math.cos(angle) * radius;
            const y = Math.sin(angle) * radius;
            if (i === 0) ctx.moveTo(x, y);
            else ctx.lineTo(x, y);
        ...
        case 'octagon':
          // Stage 7: Octagon with shield effects
          ctx.beginPath();
          for (let i = 0; i < 8; i++) {
            const angle = (Math.PI * 2 / 8) * i - Math.PI / 2;
            const radius = bossSize * 0.5;
            const x = Math.cos(angle) * radius;
            const y = Math.sin(angle) * radius;
            if (i === 0) ctx.moveTo(x, y);
            else ctx.lineTo(x, y);
        ...
        case 'prism':
          // Stage 8: Prism with rainbow colors
          const prismColors = ['#FF0000', '#FF7F00', '#FFFF00', '#00FF00', '#0000FF', '#4B0082', '#9400D3'];
          prismColors.forEach((color, i) => {
            const angle = (Math.PI * 2 / prismColors.length) * i + boss.time * 0.001;
            const radius = bossSize * 0.4;
            ctx.beginPath();
            ctx.arc(
          ...
          ctx.globalAlpha = 1;
          ctx.beginPath();
          ctx.arc(0, 0, bossSize * 0.3, 0, Math.PI * 2);
          const gradient = ctx.createRadialGradient(0, 0, 0, 0, 0, bossSize * 0.3);
          gradient.addColorStop(0, '#FFFFFF');
          gradient.addColorStop(1, boss.stage.color);
          ctx.fillStyle = gradient;
          ctx.fill();
          break;
      }
    ...
    if (player) {
      ...
      if (spaceshipImageRef.current && spaceshipImageRef.current.complete) {
        const shipWidth = player.width * 0.9;
        const shipHeight = shipWidth * 1.35; // Taller aspect ratio for proper spaceship proportions
        ctx.drawImage(
          spaceshipImageRef.current,
          -shipWidth / 2,    // x position (centered)
          -shipHeight / 2,   // y position (centered)
          shipWidth,         // width
          shipHeight         // height (taller than width for proper ratio)
        );
      } else {
        // Fallback: draw default ship if image not loaded
        ctx.beginPath();
...
{bossWarningRef.current && (
  <div className="absolute inset-0 left-0 right-0 flex items-center justify-center pointer-events-none">
    <div className="text-center">
      <div className="text-2xl font-bold text-red-500 animate-pulse title-text" style={{ textShadow: '0 0 20px #ff0000, 0 0 40px #ff0000, 0 0 60px #ff0000' }}>
        ⚠️ BOSS INCOMING! ⚠️
                </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="text-center space-y-6 p-8 bg-gradient-to-b from-slate-900 to-slate-950 rounded-2xl border border-red-500/30 shadow-2xl">
            ...
            {score >= highScore && score > 0 ? (
              <div className="text-lg text-yellow-400 title-text">🏆 NEW HIGH SCORE! 🏆: <span className="numbers-text">{score}</span></div>
            ) : (
